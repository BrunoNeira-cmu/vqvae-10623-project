...
class Grounded_VQVAE(VQVAE):
    def __init__(self, h_dim, res_h_dim, n_res_layers,
        ...
        # Load in the pretrained VQVAE and freeze CLIP Modules
        """
        TODO: Fix code to properly load in VQVAE
        """
        self._load_from_pretrained(vqvae_dict)
        self._freeze_modules()

    def _load_from_pretrained(self, vqvae_dict):
        """
    ...
    def encode_input(x):
        ...
        return text_embd.view((b,self.num_channels,res_dim_sqrt,res_dim_sqrt))
...
class Fusion_VQVAE(Grounded_VQVAE):
    def __init__(self, h_dim, res_h_dim, n_res_layers,
        ...
        """
        TODO: Load in pre-trained VQVAE
        """
        super()._freeze_modules()
    ...
    def encode_fused_input(self, x, y):
        ...
        fused_embd = self.fusion_module(text_features, img_features)
        # Get the batch size
        b, _ = text_features.shape
        # Project it into the encoder input dimension
        grounded_embd = torch.stack([layer(fused_embd) for layer in self.grounding_stack],dim = 1)
        # Return a [b, 3, res_h_dim, res_h_dim] view of our grounded text features
        return grounded_embd.view((b,self.num_channels,res_dim_sqrt,res_dim_sqrt))
...
class Embedding_VQVAE(nn.Module):
    ...
    def _freeze_modules(self):
        # Freeze CLIP
        for params in self.clip_encoder.params():
            params.requires_grad = False
        # Freeze VQVAE Encoder and Conv
        for params in self.vqvae_encoder.params():
            params.requires_grad = False
        # Freeze VQVAE Model
        for params in self.vqvae.params():
            params.requires_grad = False
